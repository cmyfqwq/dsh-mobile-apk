...
- **版本状态**：**0.13.5 开发中（vc32，2026-09-10）**：无障碍控制通道（`DeviceControlService` + 控制队列 + 双通道门禁 + 无障碍截屏）+ #126 三连修复（引擎补丁 boot-pending-G1）+ #124（引擎补丁 pi-toolcall-G2）+ #125 能力自动补全（`dsh-model-capability`，UI 全流程回归 PASS）+ 构建提速（WSL 内原生执行，172s vs ~29min）。0.13.3 已发布（vc30，Release v0.13.3 正式版，14 资产）。0.13.2 已发布（vc29，悬浮球 v2.1 全套）。当前开放跟踪：#125（已实现待发版）、#124（已修待发版）、#115（市场 Phase2）、#108（数据备份）。
