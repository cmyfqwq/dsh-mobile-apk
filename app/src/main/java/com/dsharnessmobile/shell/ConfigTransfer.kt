...
internal class MediaPickController(private val activity: MainActivity) {
  ...
  // ── 0.13.3 W10：@文件引用路径选择（文件零拷贝重构） ──────────────────────
  // SAF ACTION_OPEN_DOCUMENT → primary 存储真实路径 → 页面把 @"<path>" mention 插入
  // composer（普通 prompt 文本，内容零拷贝），模型用 read 工具按原路径读。
  // 权限链（用户拍板）：30+ 门1 All Files Access；26-28 READ/WRITE 已放行；
  // 29 与「选择工作区」同款拒绝哨兵（FUSE 原始路径 read 不可达，坑 35）。

  private var pendingFilePathPickCallback: String? = null

  private val filePathPickerBridge =
    activity.registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
      val callbackId = pendingFilePathPickCallback
      pendingFilePathPickCallback = null
      if (callbackId == null) return@registerForActivityResult
      deliverFilePathPick(callbackId, uri)
    }

  /** bridge 文件路径选择入口（AndroidBridge.pickFilePath）。 */
  fun pickFilePathForBridge(callbackId: String) {
    if (pendingFilePathPickCallback != null) {
      activity.webView.evaluateJavascript(
        "window.__dshBridge?.onFilePicked?.(" + jsString(callbackId) + ", null)", null,
      )
      return
    }
    if (android.os.Build.VERSION.SDK_INT == 29) {
      refuseFilePathPick(callbackId, "android-10")
      return
    }
    pendingFilePathPickCallback = callbackId
    try {
      filePathPickerBridge.launch(arrayOf("*/*"))
    } catch (e: Exception) {
      pendingFilePathPickCallback = null
      refuseFilePathPick(callbackId, "launcher-failed")
    }
  }

  private fun deliverFilePathPick(callbackId: String, uri: Uri?) {
    if (uri == null) {
      // 用户取消：静默（无 payload）
      activity.webView.evaluateJavascript(
        "window.__dshBridge?.onFilePicked?.(" + jsString(callbackId) + ", null)", null,
    ...
    val resolved = resolvePrimaryDocumentPath(uri)
    if (resolved == null) {
      refuseFilePathPick(callbackId, "not-local-storage")
      return
    }
    try {
      val name = queryImageName(uri) ?: resolved.substringAfterLast('/')
      val mediaType = activity.contentResolver.getType(uri) ?: ""
      val size = try {
        activity.contentResolver.openAssetFileDescriptor(uri, "r")?.use { it.length } ?: -1L
      } catch (_: Exception) { -1L }
      val json = "{\"path\":" + jsString(resolved) +
        ",\"name\":" + jsString(name) +
        ",\"size\":" + size +
        ",\"mediaType\":" + jsString(mediaType) + "}"
      activity.webView.evaluateJavascript(
        "window.__dshBridge?.onFilePicked?.(" + jsString(callbackId) + ", " + json + ")", null,
      )
    } catch (e: Exception) {
      Log.e("dsh-image", "file path pick read failed", e)
      refuseFilePathPick(callbackId, "resolve-failed")
    }
  }

  /** 拒绝哨兵（与目录选择 __dsh_pick_refused__ 家族同形）。 */
  private fun refuseFilePathPick(callbackId: String, reason: String) {
    activity.webView.evaluateJavascript(
      "window.__dshBridge?.onFilePicked?.(" + jsString(callbackId) +
        ", " + jsString("{\"refused\":\"" + reason + "\"}") + ")", null,
    )
  }

  /**
   * externalstorage document URI → primary 真实路径。
   * content://com.android.externalstorage.documents/document/primary%3ADownload%2Fx.pdf
   * → /storage/emulated/0/Download/x.pdf。其他 provider（Downloads/云盘等）无确定性
   * 真实路径 → 返回 null（页面提示改选本机存储文件）。
   */
  private fun resolvePrimaryDocumentPath(uri: Uri): String? {
    if (uri.authority != "com.android.externalstorage.documents") return null
    val doc = uri.lastPathSegment ?: return null
    val seg = try { java.net.URLDecoder.decode(doc, "UTF-8") } catch (_: Exception) { doc }
    val m = Regex("^document/primary:(.+)$").find(seg) ?: return null
