...
function findPiAiPackageDir() {
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (existsSync(join(dir, DATA_REL)))
                return dir;
        }
...
function loadRoute(dir, route) {
    const raw = JSON.parse(readFileSync(join(dir, DATA_REL, `${route}.json`), 'utf8'));
    const models = [];
    for (const [api, apiModels] of Object.entries(raw)) {
        ...
        for (const modelData of Object.values(apiModels)) {
            if (typeof modelData !== 'object' || modelData === null || typeof modelData.id !== 'string')
                continue;
 ...
 */
export function refreshLiveCatalog(routes) {
 ...
 * frozen snapshot.
 */
export function getLiveBuiltinCatalogForRoute(route) {
