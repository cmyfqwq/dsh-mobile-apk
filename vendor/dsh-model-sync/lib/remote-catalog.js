 ...
 */
export function parseRemoteCatalog(providerId, value) {
    ...
    return entries
        ...
        if (typeof entry.id !== 'string')
            return false;
        if (!('api' in entry) || typeof entry.api !== 'string')
            return false;
        return true;
    })
 ...
 */
export function loadModelsStore(storePath) {
    ...
    if (storePath === undefined && modelsStoreCache !== undefined)
        return modelsStoreCache;
    const readDoc = async () => {
        try {
            return JSON.parse(await readFile(effectivePath, 'utf8'));
        }
        catch (err) {
    ...
    const accessor = {
        ...
        update: (providerId, patch) => enqueue(async () => {
        ...
        updateOverrides: (providerId, overrides) => enqueue(async () => {
 ...
 */
export function resetModelsStoreCache() {
 ...
 */
export async function fetchRemoteCatalog(route, timeout, store, options) {
